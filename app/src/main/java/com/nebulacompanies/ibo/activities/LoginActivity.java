...
public class LoginActivity extends AppCompatActivity implements View.OnClickListener, View.OnFocusChangeListener, AsyncResponse {
    ...
    //Response<TokenModel> responseData = null;
    TokenModel responseData = null;

    public void setLoginData() {


        String token = responseData.getTokenType() + " "
                + responseData.getAccessToken();
        String refreshToken = responseData.getTokenType() + " "
                + responseData.getRefreshToken();
        String role = responseData.getRole();
        String displayName = responseData.getDisplayName();
        String ibo_key_id = responseData.getIboKeyId();
        String ibo_ref_id = responseData.getEncryptUserName();
        String ibo_userid = responseData.getUserId();
        ...
        session.setShoppyUserID(ibo_userid);
        setenablebutton(false);
    ...
    void getToken() {
        if (Utils.isNetworkAvailable(getApplicationContext())) {
            ...
            Call<TokenModel> wsCallingLogin = mAPIInterfaceShoppy.getToken(user_name, password);
            wsCallingLogin.enqueue(new Callback<TokenModel>() {
                @Override
                public void onResponse(Call<TokenModel> call, Response<TokenModel> response) {
                    if (response.isSuccessful()) {
                        if (response.code() == 200) {
                            ...
                            TokenModel responseString = response.body();
                            responseData = responseString;
                            String role = responseString.getRole();

                            if (role.equalsIgnoreCase("IBO")) {
                                //  String ibo_key_id = jsonObject.getString(Constants.WEB_SERVICES_PARAM.KEY_IBO_KEY_ID);
                                checkExpire(responseString.getIboKeyId());
                            } else {
    ...
    void setenablebutton(boolean b) {
        ...
        forgotPasswordTextView.setClickable(b);
        tvPrivacyPolicy.setClickable(b);
    }
                        ...
                        if (response.code() == 200) {
                            ...
                            if (expiredUser) {
                                ...
                                loading.setOnShowListener(dialog -> {
                                    ...
                                    textTitle.setText("Authentication Error");
    ...
    private void callMyShoppy() {
        if (Utils.isNetworkAvailable(LoginActivity.this)) {
            ...
            Call<MyShoppyModel> wsCallingEvents = mAPIInterfaceShoppy.getMyShoppy(session.getShoppyUserID());
    ...
    private void openChooseShoppyDashboard() {
        Intent i = new Intent(LoginActivity.this, ChooseShopyActivity.class);
    ...
    private void openDashboard() {
        Intent i = new Intent(LoginActivity.this, ShoppyHomeScreen.class);
                    ...
                    public void onComplete(@NonNull Task<String> task) {
                        ...
                        try {
                            ...
                            if (!TextUtils.isEmpty(TokenKey)) {
                                ...
                                Config.REGISTERED_TOKEN = TokenKey;//
    ...
    private void checkVersion() {
        ...
        if (Utils.isNetworkAvailable(getApplicationContext())) {
            try {
                Call<VersionCheck> wsCallingversionChecker = mAPIInterfaceShoppy.getVersion();
                wsCallingversionChecker.enqueue(new Callback<VersionCheck>() {
                    ...
                    public void onResponse(Call<VersionCheck> call, Response<VersionCheck> response) {
                        if (response.isSuccessful()) {
                            if (response.code() == 200) {
                                try {
                                    String latestVersions = response.body().getResponse().getData();
